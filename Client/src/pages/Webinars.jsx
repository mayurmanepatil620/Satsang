...
// Mock data for the sidebar recent posts
const recentPosts = [
  {
    id: 1,
    ...
    <div className="min-h-screen bg-[#FAF8F3] font-sans text-slate-800">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        <div className="grid lg:grid-cols-[1fr_350px] gap-16 items-start">
          ...
          {/* LEFT COLUMN: Webinar Cards */}
          <div className="grid sm:grid-cols-2 gap-8">
            {webinarsData.map((webinar) => (
              <div key={webinar.id} className="bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-500 border border-slate-100 overflow-hidden flex flex-col group relative">
                
                {/* ✅ FIXED: Image Container for Vertical Flyers */}
                <div className="h-72 w-full overflow-hidden bg-[#FAF8F3] relative flex items-center justify-center p-4">
                  <img 
                    src={webinar.image} 
                    alt={webinar.title} 
                    /* ✅ CHANGED object-cover to object-contain so flyers aren't chopped off */
                  ...
                  <div className="absolute top-4 right-4 bg-white/90 backdrop-blur-sm px-4 py-1.5 rounded-full text-xs font-bold text-[#2D1A4A] shadow-sm z-10">
                    {webinar.isFree ? "Free Access" : "Premium"}
                  </div>
                </div>

                {/* Content Container */}
                <div className="p-8 flex flex-col flex-grow text-center items-center border-t border-slate-100">
                  <h2 className="text-xl font-serif font-bold text-[#2D1A4A] mb-4 leading-snug line-clamp-2">
                    {webinar.title}
                  </h2>
                  
                  {/* Price Accent */}
                  <div className="mt-auto mb-8">
                    <p className="text-2xl font-bold text-[#B89B5E]">{webinar.price}</p>
                  </div>

                  {/* Premium Register Button */}
                  <Link to={webinar.link} className="w-full">
